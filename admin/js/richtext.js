// ====================================================
// 管理后台 · 轻量富文本编辑器（与用户中心同源逻辑）
// 暴露：window.initAdminRichTextEditors(root)
// 用法：将 textarea 加上 class="rich-text-source"，
//       渲染插入 DOM 后调用 initAdminRichTextEditors(container)。
// 设计：仅依赖 contentEditable + execCommand，无第三方库；
//       服务端已有白名单清洗，前端只做易用性+粘贴清洗+活动态高亮+快捷键+字符计数。
// ====================================================
(function() {
    'use strict';
    ...
    // 工具方法：把容器内所有富文本编辑器内容写回对应 textarea
    ...
    function initAdminRichTextEditors(root) {
        root = root || document;
        var TOOLBAR_ITEMS = [
        ...
        root.querySelectorAll('textarea.rich-text-source').forEach(function(textarea) {
            ...
            wrap.className = 'rich-text-editor admin-rich-text-editor';
    ...
    window.initAdminRichTextEditors = initAdminRichTextEditors;
    window.syncAdminRichTextEditors = syncRichTextEditors;
})();
